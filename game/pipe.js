class Pipe {
    ...
    draw () {
        this.pipesPosition.forEach(pipe => {
            image(this.image, CANVAS_WIDTH - pipe.offset, 0, PIPE_WIDTH, pipe.height, PIPE_WIDTH, 500 - pipe.height, PIPE_WIDTH, pipe.height);
            image(this.image, CANVAS_WIDTH - pipe.offset, pipe.height + pipe.gap, PIPE_WIDTH, CANVAS_HEIGHT - pipe.height - pipe.gap - FLOOROFFSET, 0, 0, PIPE_WIDTH, CANVAS_HEIGHT - pipe.height - pipe.gap - FLOOROFFSET);
        });
    }
    move () {
        this.pipesPosition.forEach(pipe => {
